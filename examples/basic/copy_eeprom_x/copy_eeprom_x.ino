    ...
    else
    {
      if ((saved_index_a != INVALID_ID) && (saved_index_b != INVALID_ID))
      {
        for (uint8_t i = 0; i < sizeof(itemList); i++)
        {
          ...
          if (i == 9)
          {
            DEBUG_SERIAL.print(dxl.readControlTableItem(itemList[i], id_a));
          }
          else if (i == 0 || i == 5 || i == 8)
          {
            DEBUG_SERIAL.print((uint8_t)dxl.readControlTableItem(itemList[i], id_a));
          }
          else
          {
            DEBUG_SERIAL.print((uint32_t)dxl.readControlTableItem(itemList[i], id_a));
          }
          ...
          if (i == 9)
          {
            DEBUG_SERIAL.println(dxl.readControlTableItem(itemList[i], id_b));
          }
          else if (i == 0 || i == 5 || i == 8)
          {
            DEBUG_SERIAL.println((uint8_t)dxl.readControlTableItem(itemList[i], id_b));
          }
          else
          {
            DEBUG_SERIAL.println((uint32_t)dxl.readControlTableItem(itemList[i], id_b));
          }
        }
      }
    }
