...
namespace DYNAMIXEL
{
  ...
  // Variable is used to store the number of model numbers in the model_number_table.
...
bool Dynamixel2Arduino::setModelNumber(uint8_t id, uint16_t model_number)
{
  bool ret = false;
...
bool Dynamixel2Arduino::setOperatingMode(uint8_t id, uint8_t mode)
{
  ...
  case MX106:
    if (mode == OP_POSITION)
    {
      if (writeControlTableItem(ControlTableItem::TORQUE_CTRL_MODE_ENABLE, id, 0) || writeControlTableItem(ControlTableItem::CW_ANGLE_LIMIT, id, 0))
        ret = writeControlTableItem(ControlTableItem::CCW_ANGLE_LIMIT, id, 4095);
    }
    else if (mode == OP_VELOCITY)
    {
    ...
    {
      if (writeControlTableItem(ControlTableItem::TORQUE_CTRL_MODE_ENABLE, id, 0) || writeControlTableItem(ControlTableItem::CW_ANGLE_LIMIT, id, 4095))
        ret = writeControlTableItem(ControlTableItem::CCW_ANGLE_LIMIT, id, 4095);
    }
...
// Not exposed to the user in Public API.
/**
 * @param model_num Model number, e.g., XL430_W250.
 * @param item_idx Control table item index, e.g., ControlTableItem::PRESENT_POSITION.
 * @param id Servo ID, e.g, 16.
 * @param data Data to be written to the control table item.
...
bool Dynamixel2Arduino::writeControlTableItem(uint16_t model_num, uint8_t item_idx, uint8_t id, int32_t data, uint32_t timeout)
{
  ...
  // Ex: model_num is XL430_W250 item_idx is ControlTableItem::PRESENT_POSITION
  item_info = getControlTableItemInfo(model_num, item_idx);
  if (item_info.addr_length > 0)
  {
...
Dynamixel2Arduino::getModelNumberIndex(uint16_t model_num)
{
  ...
  // quick shortcut, frivolous extra optimization decreasing readability
  ...
  else
  {
    ...
    {
      // pgm_read_word is used to read unsigned short
...
/**
 * @param id Servo ID, e.g., 16.
 * @return Model number, e.g., XL430_W250.
 */
...
#if (ENABLE_ACTUATOR_AX || ENABLE_ACTUATOR_DX || ENABLE_ACTUATOR_RX || ENABLE_ACTUATOR_EX)
...
#if (ENABLE_ACTUATOR_MX12W || ENABLE_ACTUATOR_MX28 || ENABLE_ACTUATOR_MX64 || ENABLE_ACTUATOR_MX106)
...
#if (ENABLE_ACTUATOR_MX28_PROTOCOL2 || ENABLE_ACTUATOR_MX64_PROTOCOL2 || ENABLE_ACTUATOR_MX106_PROTOCOL2 || ENABLE_ACTUATOR_XL330 || ENABLE_ACTUATOR_XC330 || ENABLE_ACTUATOR_XC430 || ENABLE_ACTUATOR_XL430 || ENABLE_ACTUATOR_XM430 || ENABLE_ACTUATOR_XH430 || ENABLE_ACTUATOR_XD430 || ENABLE_ACTUATOR_XM540 || ENABLE_ACTUATOR_XH540 || ENABLE_ACTUATOR_XD540 || ENABLE_ACTUATOR_XW540 || ENABLE_ACTUATOR_XW430)
...
#if (ENABLE_ACTUATOR_XM540 || ENABLE_ACTUATOR_XH540 || ENABLE_ACTUATOR_XD540 || ENABLE_ACTUATOR_XW540)
    {SET_CURRENT, GOAL_CURRENT, UNIT_MILLI_AMPERE, -2047, 2047, 2.69},
    {GET_CURRENT, PRESENT_CURRENT, UNIT_MILLI_AMPERE, -2047, 2047, 2.69},
