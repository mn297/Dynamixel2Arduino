...
bool Slave::setPacketBuffer(uint8_t *p_buf, uint16_t buf_capacity)
{
  if (p_buf == nullptr)
  {
...
bool Slave::setPort(DXLPortHandler *p_port)
{
  if (p_port == nullptr)
  {
    last_lib_err_ = DXL_LIB_ERROR_NULLPTR;
...
bool Slave::setPort(DXLPortHandler &port)
{
  p_port_ = &port;
...
void Slave::setLastLibErrCode(DXLLibErrorCode_t err_code)
{
...
bool Slave::processInstRead()
{
  ...
  if (p_rx_info->id != DXL_BROADCAST_ID)
  {
    if (p_rx_info->protocol_ver == 2)
    {
      if (p_rx_info->recv_param_len == 4)
      { // 4 = Address(2)+AddressLength(2)
    ...
    {
      if (p_rx_info->recv_param_len == 2)
      { // 2 = Address(1)+AddressLength(1)
        addr = p_rx_param[0];
    ...
    if (err == DXL_LIB_OK)
    {
      ...
      for (i = 0; i < registered_item_cnt_; i++)
      {
        ...
        if (item_addr_length != 0 && p_item->p_data != nullptr && isAddrInRange(item_start_addr, item_addr_length, addr, addr_length) == true)
        {
...
bool Slave::processInstWrite()
{
  ...
  if (err == DXL_LIB_OK)
  {
    ...
    for (i = 0; i < registered_item_cnt_; i++)
    {
      ...
      if (item_addr_length != 0 && p_item->p_data != nullptr && isAddrInRange(item_start_addr, item_addr_length, addr, data_length) == true)
      {
...
{
  if (addControlItem(ADDR_ITEM_MODEL_NUMBER, (uint16_t &)model_num_) != DXL_LIB_OK || addControlItem(ADDR_ITEM_FIRMWARE_VER, firmware_ver_) != DXL_LIB_OK || addControlItem(ADDR_ITEM_ID, id_) != DXL_LIB_OK || addControlItem(ADDR_ITEM_PROTOCOL_VER, protocol_ver_idx_) != DXL_LIB_OK)
  {
    return false;
  }
...
bool Slave::txStatusPacket(uint8_t id, uint8_t err_code, uint8_t *p_param, uint16_t param_len)
{
  bool ret = false;
  DXLLibErrorCode_t err = DXL_LIB_OK;
