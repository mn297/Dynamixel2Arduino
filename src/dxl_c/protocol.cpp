                                        ...
                                        uint8_t *p_packet_buf, uint16_t packet_buf_capacity)
{
  ...
  {
    if (id == 0xFD || id == 0xFF)
    { // http://emanual.robotis.com/docs/en/dxl/protocol2/#packet-id
      return DXL_LIB_ERROR_INVAILD_ID;
    }
    if (packet_buf_capacity < 11)
  ...
  {
    if (id == 0xFF)
    { // http://emanual.robotis.com/docs/en/dxl/protocol1/#packet-id
      return DXL_LIB_ERROR_INVAILD_ID;
    }
    if (packet_buf_capacity < 6)
...
static DXLLibErrorCode_t add_param_to_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t *p_param, uint16_t param_len)
{
  ...
  if (p_make_packet->packet_buf_capacity < generated_param_len + param_len + 6 // 6 = Header(2)+ID(1)+Length(1)+Instruction(1)+CheckSum(1) : http://emanual.robotis.com/docs/en/dxl/protocol1/#instruction-packet
      || generated_param_len + param_len > 255 - 2)
  { // Length Frame Range(1byte) = Instruction(1)+Param(N)+CheckSum(1) : http://emanual.robotis.com/docs/en/dxl/protocol1/#length
...
static DXLLibErrorCode_t add_param_to_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t *p_param, uint16_t param_len)
{
  ...
  if (p_make_packet->inst_idx == DXL_INST_STATUS)
  {
    ...
    for (i = 0; i < param_len; i++)
    {
      ...
      {
        if (p_param_buf[generated_param_len - 3] == 0xFF && p_param_buf[generated_param_len - 2] == 0xFF && p_param_buf[generated_param_len - 1] == 0xFD)
        {
      ...
      {
        if (p_packet[DXL2_0_PACKET_IDX_ERROR] == 0xFF && p_param_buf[generated_param_len - 2] == 0xFF && p_param_buf[generated_param_len - 1] == 0xFD)
        {
          p_param_buf[generated_param_len++] = 0xFD;
        }
      }
      ...
      {
        if (p_packet[DXL2_0_PACKET_IDX_INST] == 0xFF && p_packet[DXL2_0_PACKET_IDX_ERROR] == 0xFF && p_param_buf[generated_param_len - 1] == 0xFD)
        {
          p_param_buf[generated_param_len++] = 0xFD;
        }
      }
  ...
  { // Instruction Packet
    ...
    for (i = 0; i < param_len; i++)
    {
      ...
      {
        if (p_packet[DXL2_0_PACKET_IDX_INST] == 0xFF && p_param_buf[generated_param_len - 2] == 0xFF && p_param_buf[generated_param_len - 1] == 0xFD)
        {
          p_param_buf[generated_param_len++] = 0xFD;
        }
      }
...
static DXLLibErrorCode_t end_make_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet)
{
  ...
  if (p_make_packet->packet_buf_capacity < p_make_packet->param_length + 6 // 6 = Header(2)+ID(1)+Length(1)+Instruction(1)+CheckSum(1) : http://emanual.robotis.com/docs/en/dxl/protocol1/#instruction-packet
      || p_make_packet->param_length > 255 - 2)
  { // Length Frame Range(1byte) = Instruction(1)+Param(N)+CheckSum(1) : http://emanual.robotis.com/docs/en/dxl/protocol1/#length
    return DXL_LIB_ERROR_BUFFER_OVERFLOW;
  }
...
static DXLLibErrorCode_t parse_dxl1_0_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t recv_data)
{
  ...
  case DXL1_0_PACKET_PARSING_STATE_LENGTH:
    if ((uint16_t)recv_data > p_parse_packet->param_buf_capacity + 2)
    { // 2 = Instruction(1) + CheckSum(1)
  ...
  case DXL1_0_PACKET_PARSING_STATE_INST_ERR:
    ...
    p_parse_packet->calculated_check_sum += recv_data;
    p_parse_packet->recv_param_len = 0;
    if (p_parse_packet->packet_len == 2)
    { // 2 = Instruction(1) + CheckSum(1)
  ...
  case DXL1_0_PACKET_PARSING_STATE_PARAM:
    ...
    p_parse_packet->calculated_check_sum += recv_data;
    if (p_parse_packet->recv_param_len == p_parse_packet->packet_len - 2)
    { // 2 = Instruction(1) + CheckSum(1)
      p_parse_packet->parse_state = DXL1_0_PACKET_PARSING_STATE_CHECK_SUM;
    }
    break;
...
static DXLLibErrorCode_t parse_dxl2_0_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t recv_data)
{
  ...
  case DXL2_0_PACKET_PARSING_STATE_IDLE:
    ...
    if (p_parse_packet->header_cnt == 3)
    {
      if (p_parse_packet->header[0] == 0xFF && p_parse_packet->header[1] == 0xFF && p_parse_packet->header[2] == 0xFD)
      {
        p_parse_packet->recv_param_len = 0;
  ...
  case DXL2_0_PACKET_PARSING_STATE_RESERVED:
    if (recv_data != 0xFD)
    { // http://emanual.robotis.com/docs/en/dxl/protocol2/#reserved
  ...
  case DXL2_0_PACKET_PARSING_STATE_LENGTH_H:
    ...
    update_dxl_crc(&p_parse_packet->calculated_crc, recv_data);
    if (p_parse_packet->packet_len < 3)
  ...
  case DXL2_0_PACKET_PARSING_STATE_INST:
    ...
    {
      if (p_parse_packet->packet_len < 4)
      { // 4 = Instruction(1)+Error(1)+CRC(2)
        ret = DXL_LIB_ERROR_LENGTH;
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
      }
      else
  ...
  case DXL2_0_PACKET_PARSING_STATE_PARAM:
    ...
    if (p_parse_packet->inst_idx == DXL_INST_STATUS)
    {
      ...
      {
        if (p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 4] == 0xFF && p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 3] == 0xFF && p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 2] == 0xFD && p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 1] == 0xFD)
        {
      ...
      {
        if (p_parse_packet->err_idx == 0xFF && p_parse_packet->p_param_buf[0] == 0xFF && p_parse_packet->p_param_buf[1] == 0xFD && p_parse_packet->p_param_buf[2] == 0xFD)
        {
          p_parse_packet->recv_param_len--;
          byte_stuffing_cnt++;
        }
      ...
      {
        if (p_parse_packet->inst_idx == 0xFF && p_parse_packet->err_idx == 0xFF && p_parse_packet->p_param_buf[0] == 0xFD && p_parse_packet->p_param_buf[1] == 0xFD)
        {
          p_parse_packet->recv_param_len--;
          byte_stuffing_cnt++;
        }
      }
      if (p_parse_packet->recv_param_len + byte_stuffing_cnt + 4 == p_parse_packet->packet_len)
      { // 4 = Instruction(1)+Error(1)+CRC(2)
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_CRC_L;
      }
    }
    else
    {
      ...
      {
        if (p_parse_packet->inst_idx == 0xFF && p_parse_packet->p_param_buf[0] == 0xFF && p_parse_packet->p_param_buf[1] == 0xFD && p_parse_packet->p_param_buf[2] == 0xFD)
        {
          p_parse_packet->recv_param_len--;
          byte_stuffing_cnt++;
        }
      }
      if (p_parse_packet->recv_param_len + byte_stuffing_cnt + 3 == p_parse_packet->packet_len)
      { // 3 = Instruction(1)+CRC(2)
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_CRC_L;
      }
    }
